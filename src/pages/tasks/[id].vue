...
watch(
  () => task.value?.name,
  () => (usePageStore().pageData.title = `Task: ${task.value?.name || null}`)
)

const getTask = async () => {
  const { data, error } = await taskWithProjectQuery(Number(route.params.id))

  if (error) {
    console.error('Could not fetch task')
  }
      ...
      <TableCell>
        <div class="flex">
          <Avatar
            ...
            v-for="colab in task.collaborators"
            :key="colab"
          >
